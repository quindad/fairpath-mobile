...
export function JobMap({jobs,onOpenJob,compact=false}:{jobs:Job[];onOpenJob?:(job:Job)=>void;compact?:boolean}){
 ...
 return <View style={[s.wrap,compact&&s.wrapCompact]}>
  <MapView
   ...
   initialRegion={{latitude:first.latitude!,longitude:first.longitude!,latitudeDelta:compact?.08:.18,longitudeDelta:compact?.08:.18}}
  >
   {mapped.map(j=><Marker
    key={j.id}
  ...
  <View style={s.note}><Text style={s.noteText}>{compact?'Approximate work area · exact location may be shared by the employer':'Pins use the best location FairPath has. City-level pins are approximate.'}</Text></View>
 </View>;
}
