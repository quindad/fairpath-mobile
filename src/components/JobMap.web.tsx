...
export function JobMap({jobs,compact=false}:{jobs:Job[];onOpenJob?:(job:Job)=>void;compact?:boolean}){
 ...
 const iframe=React.createElement('iframe' as any,{srcDoc:html,title:'FairPath Jobs Map',style:{width:'100%',height:compact?'220px':'430px',border:'0',display:'block'}});
 return <View style={[s.wrap,compact&&s.wrapCompact]}>{iframe}<View style={s.note}><Text style={s.noteText}>{compact?'Approximate work area · exact location may be shared by the employer':'Pins use the best location FairPath has. City-level pins are approximate.'}</Text></View></View>;
}
