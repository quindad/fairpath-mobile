...
export function HousingApplicationDocuments({applicationId,readOnly=false}:{applicationId:string;readOnly?:boolean}){
 ...
 async function load(){setLoading(true);try{setRows(await loadHousingApplicationDocuments(applicationId))}catch{}finally{setLoading(false)}}
 ...
 async function remove(doc:HousingApplicationDocument){
  Alert.alert('Remove document?','This removes the uploaded file from this application.',[
   ...
   {text:'Remove',style:'destructive',onPress:async()=>{try{await deleteHousingApplicationDocument(doc);setRows(v=>v.filter(x=>x.id!==doc.id))}catch{Alert.alert('Could not remove','Please try again.')}}}
  ]);
 }
