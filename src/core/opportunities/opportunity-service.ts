...
export async function submitHousingApplication(applicationId:string,form:HousingApplicationForm){const user=await currentUser();const now=new Date().toISOString();const {error}=await supabase.from('housing_applications').update({answers:form,current_step:5,status:'submitted',submitted_at:now,updated_at:now,applicant_snapshot:form,consent_snapshot:{confirmed:true,confirmed_at:now}}).eq('id',applicationId).eq('user_id',user.id).eq('status','started');if(error)throw error;}
