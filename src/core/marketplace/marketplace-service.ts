...
export type MarketplaceClaim={
 id:string;item_id:string;status:MarketplaceClaimStatus;pickup_deadline:string|null;pickup_code:string|null;
...
export type MarketplaceClaimCandidate={id:string;status:MarketplaceClaimStatus;claimant_label:string;created_at:string;pickup_deadline:string|null};
...
export async function sendMarketplaceMessage(claimId:string,body:string){await currentUser();const text=body.trim();if(!text)throw new Error('MESSAGE_REQUIRED');const {data,error}=await supabase.rpc('send_marketplace_pickup_message',{p_claim_id:claimId,p_body:text});if(error)throw error;const row=Array.isArray(data)?data[0]:data;if(!row)throw new Error('MESSAGE_NOT_SENT');return row as MarketplaceMessage}
