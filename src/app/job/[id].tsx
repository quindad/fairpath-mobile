...
export default function JobDetail(){
 ...
 async function apply(){if(!job)return;if(job.application_method==='demo'){Alert.alert('Preview job','This is sample data for the FairPath preview. Live jobs can use FairPath Easy Apply.');return;}if(job.application_method==='external'&&job.external_apply_url){await Linking.openURL(job.external_apply_url);return}router.push(('/job-apply/'+job.id) as never)}
 ...
 return <ScreenFrame><PageHeader eyebrow="FAIRPATH JOBS" title="Job details" trailing={<Pressable style={s.save} onPress={save}><Text style={s.saveText}>SAVE</Text></Pressable>}/>
 <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
  <Text style={s.company}>{job.company_name.toUpperCase()}</Text><Text style={s.title}>{job.title}</Text>
  <View style={s.locationRow}><Text style={s.location}>{location||'Location not listed'}</Text>{job.location_precision==='city'?<Text style={s.approx}>APPROX. AREA</Text>:null}</View>
  <View style={s.badges}><InlineBadge tone={second?'lime':'default'}>{second?'VERIFIED SECOND-CHANCE':'POLICY REVIEW NEEDED'}</InlineBadge><InlineBadge>{job.workplace_type.toUpperCase()}</InlineBadge><InlineBadge>{job.employment_type.replace('_',' ').toUpperCase()}</InlineBadge></View>
  {job.pay_min!=null?<View style={s.fact}><Text style={s.factLabel}>PAY</Text><Text style={s.factValue}>{'
 ...
 <View style={s.bottom}><Pressable style={s.apply} onPress={apply}><Text style={s.applyText}>{job.application_method==='demo'?'PREVIEW LISTING':job.application_method==='external'?'CONTINUE TO APPLY':job.easy_apply_enabled?'EASY APPLY WITH FAIRPATH':'APPLY WITH FAIRPATH'}</Text><Text style={s.applyText}>→</Text></Pressable></View>
 </ScreenFrame>
}
const s=StyleSheet.create({
 content:{paddingHorizontal:L.mobileGutter,paddingTop:20,paddingBottom:110},state:{padding:L.mobileGutter},stateText:{color:C.muted},error:{color:C.danger},
 save:{height:32,borderRadius:R.xs,borderWidth:1,borderColor:C.borderStrong,paddingHorizontal:10,justifyContent:'center'},saveText:{color:C.white,fontFamily:F.extraBold,fontSize:8,letterSpacing:.8},
 company:{color:C.lime,fontFamily:F.extraBold,fontSize:9,letterSpacing:1.2},title:{color:C.white,fontFamily:F.black,fontSize:31,lineHeight:33,letterSpacing:-.9,marginTop:7},
 locationRow:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginTop:10},location:{color:C.mutedStrong,fontSize:13},approx:{color:C.lime,fontFamily:F.extraBold,fontSize:7,letterSpacing:.8},badges:{flexDirection:'row',gap:6,flexWrap:'wrap',marginTop:15},
 fact:{paddingVertical:18,borderBottomWidth:1,borderBottomColor:C.border},factLabel:{color:C.muted,fontFamily:F.extraBold,fontSize:8,letterSpacing:1},factValue:{color:C.white,fontFamily:F.extraBold,fontSize:17,marginTop:5},
 mapSection:{paddingVertical:18,borderBottomWidth:1,borderBottomColor:C.border},section:{paddingVertical:18,borderBottomWidth:1,borderBottomColor:C.border},sectionLabel:{color:C.muted,fontFamily:F.extraBold,fontSize:8,letterSpacing:1.1,marginBottom:8},body:{color:C.mutedStrong,fontSize:13,lineHeight:20},line:{color:C.mutedStrong,fontSize:13,lineHeight:21},
 ...
 apply:{height:48,borderRadius:R.sm,backgroundColor:C.lime,paddingHorizontal:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},applyText:{color:C.black,fontFamily:F.extraBold,fontSize:10,letterSpacing:.8}
});+Number(job.pay_min).toLocaleString()+(job.pay_max?' – 
  {job.background_policy_summary?<View style={s.section}><Text style={s.sectionLabel}>BACKGROUND POLICY</Text><Text style={s.body}>{job.background_policy_summary}</Text></View>:null}
  {job.requirements?.length?<View style={s.section}><Text style={s.sectionLabel}>REQUIREMENTS</Text>{job.requirements.map(x=><Text key={x} style={s.line}>— {x}</Text>)}</View>:null}
  {job.skills?.length?<View style={s.section}><Text style={s.sectionLabel}>SKILLS</Text><Text style={s.body}>{job.skills.join(' · ')}</Text></View>:null}
  {job.benefits?.length?<View style={s.section}><Text style={s.sectionLabel}>BENEFITS</Text><Text style={s.body}>{job.benefits.join(' · ')}</Text></View>:null}
...
const s=StyleSheet.create({
 ...
 save:{height:32,borderRadius:R.xs,borderWidth:1,borderColor:C.borderStrong,paddingHorizontal:10,justifyContent:'center'},saveText:{color:C.white,fontFamily:F.extraBold,fontSize:8,letterSpacing:.8},
 company:{color:C.lime,fontFamily:F.extraBold,fontSize:9,letterSpacing:1.2},title:{color:C.white,fontFamily:F.black,fontSize:31,lineHeight:33,letterSpacing:-.9,marginTop:7},
 location:{color:C.mutedStrong,fontSize:13,marginTop:10},badges:{flexDirection:'row',gap:6,flexWrap:'wrap',marginTop:15},
 fact:{paddingVertical:18,borderBottomWidth:1,borderBottomColor:C.border},factLabel:{color:C.muted,fontFamily:F.extraBold,fontSize:8,letterSpacing:1},factValue:{color:C.white,fontFamily:F.extraBold,fontSize:17,marginTop:5},
 section:{paddingVertical:18,borderBottomWidth:1,borderBottomColor:C.border},sectionLabel:{color:C.muted,fontFamily:F.extraBold,fontSize:8,letterSpacing:1.1,marginBottom:8},body:{color:C.mutedStrong,fontSize:13,lineHeight:20},line:{color:C.mutedStrong,fontSize:13,lineHeight:21},
 source:{paddingVertical:16},sourceLabel:{color:C.muted,fontFamily:F.extraBold,fontSize:8,letterSpacing:1},sourceText:{color:C.mutedStrong,fontSize:11,marginTop:4},
 bottom:{position:'absolute',left:0,right:0,bottom:0,paddingHorizontal:L.mobileGutter,paddingTop:10,paddingBottom:14,backgroundColor:C.black,borderTopWidth:1,borderTopColor:C.border},
 apply:{height:48,borderRadius:R.sm,backgroundColor:C.lime,paddingHorizontal:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},applyText:{color:C.black,fontFamily:F.extraBold,fontSize:10,letterSpacing:.8}
});+Number(job.pay_max).toLocaleString():'')+' / '+(job.pay_period||'period')}</Text></View>:null}
  {job.latitude!=null&&job.longitude!=null?<View style={s.mapSection}><Text style={s.sectionLabel}>WORK AREA</Text><JobMap jobs={[job]} compact/></View>:null}
