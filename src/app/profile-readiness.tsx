...
const areas=[
 ['Identity','Basic information used for autofill and nearby opportunities.','40%'],
 ['Employment','Skills, education, work goals and transportation.','25%'],
 ['Housing','Household, income and where you want to live.','20%'],
 ['Reentry','Release and supervision information for your FairPath Forward plan.','50%'],
 ['Documents','Know what you have and what you still need.','20%'],
 ['Eligibility','Information used to improve compatibility and program screening.','30%'],
];
...
export default function ProfileReadiness(){
 return <View style={s.screen}><StatusBar style="light"/><SafeAreaView style={s.safe}>
  <ScrollView contentContainerStyle={s.content}>
   ...
   <View style={s.overall}><View style={s.overallTop}><Text style={s.overallTitle}>Overall readiness</Text><Text style={s.percent}>32%</Text></View><View style={s.track}><View style={s.fill}/></View><Text style={s.note}>We’ll only ask follow-up questions that apply to you.</Text></View>
   <Text style={s.section}>COMPLETE YOUR FAIRPATH</Text>
   {areas.map(([title,body,pct])=><Pressable key={title} style={s.card} onPress={()=>router.push('/onboarding' as never)}><View style={s.cardTop}><Text style={s.cardTitle}>{title}</Text><Text style={s.cardPct}>{pct}</Text></View><Text style={s.cardBody}>{body}</Text><View style={s.cardBottom}><Text style={s.continue}>Continue section</Text><Text style={s.arrow}>→</Text></View></Pressable>)}
