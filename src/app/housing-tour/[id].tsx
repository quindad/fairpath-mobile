...
import { formatDateInput, isValidDateText } from '@/core/forms/formatters';
...
export default function HousingTour(){
 ...
 async function submit(){if(!id||saving)return;if(!isValidDateText(date,{allowFuture:true})){Alert.alert('Choose a valid date','Enter the preferred tour date as MM/DD/YYYY.');return}const [m,d,y]=date.split('/');setSaving(true);try{await createHousingTourRequest({listingId:id,preferredDate:`${y}-${m}-${d}`,preferredWindow:window,note});Alert.alert('Tour request sent','Your request is saved in FairPath. A property partner can confirm or decline it when Partner tools are live.');router.back()}catch(e){if(e instanceof Error&&e.message==='SIGNED_OUT'){router.push(('/sign-up?returnTo='+encodeURIComponent('/housing-tour/'+id)) as never);return}Alert.alert('Could not request tour','Please try again.')}finally{setSaving(false)}}
 return <ScreenFrame><PageHeader eyebrow="FAIRPATH HOUSING" title="Request a tour" backTo={id?'/housing/'+id:'/find-housing'}/><ScrollView contentContainerStyle={s.content}>
  ...
  <Text style={s.label}>PREFERRED DATE</Text><TextInput style={s.input} value={formatDateInput(date)} onChangeText={v=>setDate(formatDateInput(v))} keyboardType="number-pad" maxLength={10} placeholder="MM/DD/YYYY" placeholderTextColor={C.muted}/>
