...
export default function FindJobs(){
 ...
 return <ScreenFrame>
  ...
  <View style={s.filtersStrip}><View style={s.filterCell}><SharpChip label="Remote" active={remote} onPress={chooseRemote}/></View><View style={s.filterCell}><SharpChip label="Full-time" active={fullTime} onPress={chooseFull}/></View><View style={s.filterCell}><SharpChip label="Part-time" active={partTime} onPress={choosePart}/></View></View>
  <ScrollView contentContainerStyle={s.list} showsVerticalScrollIndicator={false}>
   ...
   {jobs.map(j=><Pressable key={j.id} style={s.card} onPress={()=>openJob(j)}>
    <View style={s.cardTop}>
     ...
     <View style={s.cardTopCopy}><Text style={s.jobTitle}>{j.title}</Text><Text style={s.company}>{j.company_name}</Text></View>
     <Pressable accessibilityRole="button" accessibilityLabel="Save job" style={s.saveBtn} onPress={(e)=>{e.stopPropagation?.();void handleSave(j.id)}}><Lucide name="bookmark" color={C.mutedStrong} size={15}/></Pressable>
    </View>
    <View style={s.metaRow}><Lucide name="map-pin" color={C.muted} size={13}/><Text style={s.meta}>{j.location_text||[j.city,j.state].filter(Boolean).join(', ')||'Location not listed'}</Text><View style={s.metaDot}/><Text style={s.meta}>{j.workplace_type.replace('_',' ')}</Text></View>
...
const s=StyleSheet.create({
 ...
 companyMark:{width:36,height:36,borderRadius:2,borderWidth:1,borderColor:C.borderStrong,backgroundColor:'#0A0C0A',alignItems:'center',justifyContent:'center',marginRight:10},companyMarkText:{color:C.white,fontFamily:F.extraBold,fontSize:13},
 ...
 jobTitle:{color:C.white,fontFamily:F.extraBold,fontSize:20,lineHeight:22,letterSpacing:-.4},company:{color:C.mutedStrong,fontFamily:F.bold,fontSize:12,marginTop:3},
 metaRow:{flexDirection:'row',alignItems:'center',flexWrap:'wrap',gap:6,marginTop:12},meta:{color:C.muted,fontSize:11,textTransform:'capitalize'},metaDot:{width:3,height:3,borderRadius:2,backgroundColor:C.borderStrong},
 pay:{color:C.white,fontFamily:F.extraBold,fontSize:14,marginTop:11},badgeRow:{flexDirection:'row',gap:6,flexWrap:'wrap',marginTop:11},
 desc:{color:C.mutedStrong,fontSize:11,lineHeight:17,marginTop:13},cardFooter:{marginTop:15,paddingTop:12,borderTopWidth:1,borderTopColor:C.border,flexDirection:'row',justifyContent:'space-between',alignItems:'center'},
 sourceLabel:{color:C.muted,fontFamily:F.extraBold,fontSize:7,letterSpacing:1},source:{color:C.mutedStrong,fontSize:9,marginTop:3},viewAction:{flexDirection:'row',alignItems:'center',gap:6},viewText:{color:C.lime,fontFamily:F.extraBold,fontSize:8,letterSpacing:.9},
...
+Number(j.pay_min).toLocaleString()+(j.pay_max?' – 
  </ScrollView>
 </ScreenFrame>
}
const s=StyleSheet.create({
...
+Number(j.pay_max).toLocaleString():'')+' / '+(j.pay_period||'period')}</Text>:null}
    ...
    <View style={s.cardFooter}><View><Text style={s.sourceLabel}>SOURCE</Text><Text style={s.source}>{j.source_label||'FairPath'}</Text></View><View style={s.viewAction}><Text style={s.viewText}>VIEW JOB</Text><Lucide name="arrow-right" color={C.lime} size={14}/></View></View>
   </Pressable>)}
  </ScrollView>
 </ScreenFrame>
}
